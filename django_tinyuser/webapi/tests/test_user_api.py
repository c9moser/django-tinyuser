...
class PrivateUserApiTests(TestCase):
    ...
    def test_patch_user_success(self):
        ...
        # Password should not be updated here since it's not included in the response data
...
class TokenAPITests(TestCase):
    ...
    def test_create_token_for_user_valid_credentials(self):
        ...
        create_user(
            email=payload["email"], password=payload["password"], username="TestUser"
        )
    ...
    def test_create_token_for_user_missing_fields(self):
        ...
        res = self.client.post(
            TOKEN_URL, {"email": "", "password": user_data["password"]}
        )
        self.assertNotIn("token", res.data)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
