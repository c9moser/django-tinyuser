...
from django_tinyuser.settings import TEMPLATE_MAPPING, TEMP_DIR
...
class ProfileView(LoginRequiredMixin, HtmxMixin, FormView):
    ...
    def save_avatars(self, instance: TinyUserProfile, files: dict):
        ...
        avatar_small_media = Path(django_settings.MEDIA_ROOT) / "user/avatars" / avatar_small_name
        ...
        avatar_medium_media = Path(django_settings.MEDIA_ROOT) / "user/avatars" / avatar_medium_name
        ...
        avatar_large_media = Path(django_settings.MEDIA_ROOT) / "user/avatars" / avatar_large_name
        ...
        avatar_full_media = Path(django_settings.MEDIA_ROOT) / "user/avatars" / avatar_full_name
        ...
        if avatar_small_img:
            ...
            with open(avatar_small_path, 'rb') as avatar_small:
                instance.avatar_small.save(avatar_small_path.name, ImageFile(avatar_small))
        ...
        if avatar_medium_img:
            ...
            with open(avatar_medium_path, 'rb') as avatar_medium:
                instance.avatar_medium.save(avatar_medium_path.name, ImageFile(avatar_medium))
        ...
        if avatar_large_img:
            ...
            with open(avatar_large_path, 'rb') as avatar_large:
                instance.avatar_large.save(avatar_large_path.name, ImageFile(avatar_large))
        ...
        for temp_file in [avatar_small_path, avatar_medium_path, avatar_large_path, avatar_full_path]:
            if temp_file.exists():
                temp_file.unlink()
